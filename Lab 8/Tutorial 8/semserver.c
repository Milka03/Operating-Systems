...
void* communicateDgram(void *arg){
        ...
        if(TEMP_FAILURE_RETRY(sendto(args->fd,(char *)(&(args->time)),sizeof(int16_t),0,&(args->addr),sizeof(args->addr)))<0) ERR("sendto");
...
void doServer(int fd){
        ...
        while(do_work){
                ...
                if (TEMP_FAILURE_RETRY(sem_trywait(&semaphore)) == -1) {
                        switch(errno){
                        case EAGAIN:    if(TEMP_FAILURE_RETRY(sendto(fd,(char *)&deny,sizeof(int16_t),0,&addr,sizeof(addr)))<0) ERR("sendto");
                        case EINTR:     continue;
                        }
                        ERR("sem_wait");
                }
